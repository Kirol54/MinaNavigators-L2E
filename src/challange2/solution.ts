...
export const ProcessMessageBatch = ZkProgram({
  ...
  methods: {
    initBatch: {
      privateInputs: [],
      async method(lowestMsgNumber: Field) {
...
export const processBatchSequentiallySized = async (
  msgs: Array<Message>,
  batchSize: number
) => {
...
export const processBatchSequentially = async (
  msgs: Array<Message>,
  initialProof?: ProcessMessageBatchProof
) => {
  if (!msgs) throw new Error('No messages provided!');

  let proof =
    initialProof || (await ProcessMessageBatch.initBatch(new Field(0)));
...
export class MessageBoxContract extends SmartContract {
  ...
  @method
  async processBatch(batchProcessProof: ProcessMessageBatchProof) {
    batchProcessProof.verify();
    const cHigh = this.highestProcessedMessage.getAndRequireEquals();
    const pHigh = batchProcessProof.publicOutput.highestMsgNumber;
    const newHighest = Provable.if(pHigh.greaterThan(cHigh), pHigh, cHigh);
