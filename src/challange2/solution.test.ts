// SOLUTION BASED ON https://github.com/adamczykm/mina-navigator-challenges/tree/main/February
import { jest } from '@jest/globals';
import { Bool, PrivateKey, Field, Mina, AccountUpdate, PublicKey } from 'o1js';
...
export const getDeployMessageBoxTx = async (keys: {
  ...
  console.log('Compiling MessageBox programs');
  ...
  console.log('Creating the deploy tx');
  const tx = await Mina.transaction(sender, async () => {
    AccountUpdate.fundNewAccount(sender);
    zkapp.deploy();
  });
  console.log('Proving the deploy tx');
  await tx.prove();
  return { zkapp, tx };
};

export const setupEnv = async (opts: { proofsEnabled: boolean }) => {
  let Local = await Mina.LocalBlockchain({ proofsEnabled: opts.proofsEnabled });
  Mina.setActiveInstance(Local);
  const owner = Local.testAccounts[0],
    ownerKey = owner.key;
  ...
  console.log('Signing by deployer and sending the deployment tx');
...
export const processBatchInChunks = async (
  ...
  console.log('Creating the proof of processing batch of messages in chunks');
  const batchProof = await processBatchSequentiallySized(msgs, chunkSize);

  console.log('Building the `processBatch` tx');
  const tx = await Mina.transaction(owner, async () => {
    zkapp.processBatch(batchProof);
  });
  console.log('Proving the `processBatch` tx');
  await tx.prove();
  const retTx = tx.sign([zkappKey, ownerKey]);

  console.log('Signing and sending the `processBatch` tx');
...
const mkValidMessage = (
  ...
  console.log('Valid message', ret);
  console.log('Valid message details', details(ret));
...
jest.mock('./solution', () => {
  ...
  const mockedCheckSumFails = jest.fn((...args: [MessageDetails]) =>
    originalModule.checkSumFails(...args)
  );
...
describe(`Testing Challenge2 - The SpyMaster's Message Box`, () => {
  beforeAll(async () => {
    console.log('Starting MessageBox tests');
    console.log('Compiling processMessageBatch program...');
    // let { verificationKey } =
    //   await ProcessMessageBatch.compile();
  }),
